...
from gotta.notes.model import ActorNotesPayload, SessionNoteRecord, SessionNotesPayload
...
def show_notes(args: argparse.Namespace) -> int:
    ...
    if scoped_actor:
        ...
        if args.output == "json":
            print(json.dumps(actor_payload, indent=2, sort_keys=True))
            return 0
        print(
    ...
    actor_payloads: dict[str, ActorNotesPayload] = {}
    ...
    for actor_name in actor_ids:
        ...
        for record in actor_payload["entries"]:
            session_record: SessionNoteRecord = {
                **record,
                "actor": record["actor"] or actor_name,
                "label": session_registry._actor_label(actor_name, work_dir=work_dir),
            }
            entries.append(session_record)
